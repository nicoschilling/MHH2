...
u_experimentspecifier=${experimentspecifier?Error: no experimentspecifier defined!}
...
tablename=${u_experimentspecifier}
...
psql -A -t -d ${database} -U ${dbuser} -c "${queryAccuracy}" -o ${modelfileAccuracy}/parameters  
psql -A -t -d ${database} -U ${dbuser} -c "${querySampleDiff}" -o ${modelfileSampleDiff}/parameters
